...
namespace Test
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            db.GetAllData2(Trading.IEXData.HistoryType.FiveYear, 200);
                    ...
                    foreach (Company c in list)
                    {
                        ...
                        if (found)
                        {
                            if (c.IsHighToDate(ind)) //Search for A
                            {
                                ...
                                c.CurrentCupHandle.A = c.GetPoint(ind, thisDay); //Set new A
                            }
                            else if (c.CurrentCupHandle.A != null)
                            {
                                int lastA = ind - c.CurrentCupHandle.A.Index;
                                ...
                                if (chp.AC.ContainsValue(lastA)) //Last A in correct Range, search for C
                                {
                                    ...
                                    if (chp.PivotRatio.ContainsValue(possibleC.Close / c.CurrentCupHandle.A.Close))
                                    {
                                        ...
                                        {
                                            c.CurrentCupHandle.C = possibleC;
                                            ...
                                            if (bestR1R3 > 0)
                                            {
                                                ...
                                                c.CurrentCupHandle.B = c.GetPoint(bestR1R3Index, bDate);

                                                //Now continue and find PointD the handle
                                                int rangeCD = chp.Handle.Maximum - chp.Handle.Minimum;
                                                int startDsearch = c.CurrentCupHandle.C.Index;
